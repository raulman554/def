...
use App\Models\LogActividadSistema;
...
public function destroy($id)
{
    try {
        ...
        $ticketsVendidos = $instancia->tickets()->where('estado', 'vendido')->count();
        
        // Si hay tickets vendidos y no se ha forzado la eliminación
        if ($ticketsVendidos > 0 && !request()->has('force_delete')) {
            return back()->with('warning', 
                ...
                <form action='" . route('instancias-sorteo.destroy', $instancia->id) . "' method='POST'>
                    " . csrf_field() . "
                    " . method_field('DELETE') . "
                    <input type='hidden' name='force_delete' value='1'>
                    <button type='submit' class='btn btn-danger'>Forzar eliminación</button>
                </form>");
        }
        ...
        return back()->with('error', 'No se pudo eliminar la instancia: ' . $e->getMessage());
    }
}
